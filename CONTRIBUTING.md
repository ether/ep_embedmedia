# Contributing to ep_embedmedia

Thanks for helping improve `ep_embedmedia` — a plugin for [Etherpad](https://etherpad.org/). LLM/Agent contributions are explicitly welcome, provided they follow the rules below.
...
`ep_embedmedia` is a plugin, so you develop it inside a checkout of [ether/etherpad](https://github.com/ether/etherpad).
...
# Install ep_embedmedia from your local clone
pnpm run plugins i --path ../ep_embedmedia

# Start the dev server (port 9001)
pnpm --filter ep_etherpad-lite run dev
```

Edits in `../ep_embedmedia/` are picked up after a server restart.
...
* Etherpad version, ep_embedmedia version, browser + OS
* Steps to reproduce
